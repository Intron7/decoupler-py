...
def _net_2_g(
    ...
    for i, edge in enumerate(edges):
        g.edges[edge]["weight"] = net["weight"].values[i]

    # Update node attributes
    node_labels = list(v_dict.keys())
    for i, node in enumerate(g.nodes()):
        g.nodes[node]["type"] = types[i]
        g.nodes[node]["label"] = node_labels[i]
        g.nodes[node]["shape"] = "o" if types[i] else "s"  # circle for targets, square for sources
...
def _gcolors(
    ...
    if (s_cmap in cmaps) and (t_cmap in cmaps):
        s_cmap_obj = matplotlib.colormaps.get_cmap(s_cmap)
        t_cmap_obj = matplotlib.colormaps.get_cmap(t_cmap)
        for node in g.nodes():
            node_label = g.nodes[node]["label"]
            if g.nodes[node]["type"]:
                g.nodes[node]["color"] = t_cmap_obj(t_norm(data[node_label].values[0]))
            else:
                g.nodes[node]["color"] = s_cmap_obj(s_norm(score[node_label].values[0]))
        is_cmap = True
    else:
        for node in g.nodes():
            g.nodes[node]["color"] = s_cmap if g.nodes[node]["type"] == 0.0 else t_cmap
        is_cmap = False
    return is_cmap
...
def network(
    ...
    **kwargs,
    ...
    # Color norms
    s_norm = _norm(x=fscore.values, vcenter=vcenter)
    t_norm = _norm(x=fdata.values, vcenter=vcenter)
    # Make graph
    g = _net_2_g(data=fdata, score=fscore, net=fnet)
    # Set edge colors
    for edge in g.edges():
        weight = g.edges[edge]["weight"]
        g.edges[edge]["color"] = c_pos_w if weight > 0 else c_neg_w
    ...
    # Plot nodes with different shapes
    sources = [node for node in g.nodes() if g.nodes[node]["type"] == 0]
    targets = [node for node in g.nodes() if g.nodes[node]["type"] == 1]

    if sources:
        source_colors = [g.nodes[node]["color"] for node in sources]
        nx.draw_networkx_nodes(
            g, pos, nodelist=sources, node_color=source_colors, node_shape="s", node_size=size_node * 100, ax=ax1
        )

    if targets:
        target_colors = [g.nodes[node]["color"] for node in targets]
        nx.draw_networkx_nodes(
            g, pos, nodelist=targets, node_color=target_colors, node_shape="o", node_size=size_node * 100, ax=ax1
        )

    # Draw edges
    nx.draw_networkx_edges(g, pos, edge_color=edge_colors, ax=ax1, arrows=True, arrowsize=20)

    # Draw labels
    nx.draw_networkx_labels(g, pos, labels=node_labels, font_size=size_label * 4, ax=ax1)
    if is_cmap:
